...
class TestRelib(unittest.TestCase):
    ...
    def test_users(self):
        ...
        access_key = gen_salt(32)
        secret_key = gen_salt(32)
        autogenerated_password = gen_salt(32)

        service_user = ts.get_table('users').add({
            'organization_name': 'directivegames',
            'user_name': 'ue4server',
            'is_service': True,
        })
        ...
        service_user = ts.get_table('users').add({
            ...
            'secret_key': generate_password_hash(secret_key, method='pbkdf2:sha1:25000'),
            'password': generate_password_hash(autogenerated_password, method='pbkdf2:sha1:25000')
        })
        self.assertTrue(check_password_hash(service_user['secret_key'], secret_key))
        self.assertTrue(check_password_hash(service_user['password'], autogenerated_password))
